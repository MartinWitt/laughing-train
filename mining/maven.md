...
# Bad smells
I found 1360 bad smells with 136 repairable:
| ruleID | number | fixable |
| --- | --- | --- |
| ReturnNull | 230 | false |
| BoundedWildcard | 173 | false |
| UnnecessaryFullyQualifiedName | 111 | false |
| NonSynchronizedMethodOverridesSynchronizedMethod | 76 | false |
| AssignmentToMethodParameter | 75 | false |
...
| SystemOutErr | 33 | false |
| ConstantValue | 32 | false |
| UseOfPropertiesAsHashtable | 30 | false |
...
Iteration over `issues.mojoIssues.keySet()` may be replaced with 'entrySet()' iteration
in `maven-core/src/main/java/org/apache/maven/plugin/internal/DefaultPluginValidationManager.java`
#### Snippet
```java
...
### SizeReplaceableByIsEmpty
`projectBuilds.size() > 0` can be replaced with '!projectBuilds.isEmpty()'
...
### BoundedWildcard
Can generalize to `? extends MavenPluginDependenciesValidator`
in `maven-core/src/main/java/org/apache/maven/plugin/internal/DefaultMavenPluginManager.java`
#### Snippet
```java
            MavenPluginValidator pluginValidator,
...
### NestedAssignment
Result of assignment expression used
in `maven-core/src/main/java/org/apache/maven/internal/transformation/TransformedArtifact.java`
#### Snippet
```java
...
### EmptyMethod
All implementations of this method are empty
in `maven-model-builder/src/main/java/org/apache/maven/model/plugin/ReportingConverter.java`
#### Snippet
```java
...
in `log/indexing-diagnostic/project.15375f63/diagnostic-2023-04-21-22-16-04.188.html`
#### Snippet
```java
              <td>0</td>
              <td>0</td>
...
Class `XmlNodeWriter` has only 'static' members, and lacks a 'private' constructor
in `maven-xml-impl/src/main/java/org/apache/maven/internal/xml/XmlNodeWriter.java`
#### Snippet
```java
 *
 */
...
### UtilityClassWithoutPrivateConstructor
Class `XmlNodeBuilder` has only 'static' members, and lacks a 'private' constructor
...
### DynamicRegexReplaceableByCompiledPattern
`replace()` could be replaced with compiled 'java.util.regex.Pattern' construct
in `maven-core/src/main/java/org/apache/maven/plugin/internal/AbstractMavenPluginDescriptorSourcedParametersValidator.java`
#### Snippet
```java
        }
...
### UnnecessaryFullyQualifiedName
Qualifier `org.apache.maven.artifact.repository` is unnecessary, and can be replaced with an import
...
### AssignmentToMethodParameter
Assignment to method parameter `trace`
in `maven-core/src/main/java/org/apache/maven/internal/aether/ReverseTreeRepositoryListener.java`
#### Snippet
```java
                break;
...
### ReturnNull
Return of `null`
in `maven-core/src/main/java/org/apache/maven/internal/transformation/OnChangeTransformer.java`
#### Snippet
```java
