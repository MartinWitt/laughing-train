    ...
    <div>
      ...
      <Grid2 container padding={5}>
        ...
        <Grid2 xs={2}>
          ...
          <Button variant="contained" size="large" value={""} onClick={() => {
            refactorBadSmell({
              variables: {
...
function StackSelector({ dataBadSmell, handleSelect }: { dataBadSmell: any; handleSelect: (value: string[]) => void; }): JSX.Element {
  ...
  return <Stack spacing={0.5}>
    {listOfUniqueRules(badSmells).map((badSmell: BadSmell) => {
      return <Chip key={badSmell.ruleID} label={badSmell.ruleID} onClick={(e) => handleSelect(toRuleIDs(filterBadSmells(badSmells, e.currentTarget.innerText)))} />;
    })}
  </Stack>;
}
