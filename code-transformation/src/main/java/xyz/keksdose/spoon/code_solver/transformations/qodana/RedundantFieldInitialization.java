...
public class RedundantFieldInitialization extends TransformationProcessor<CtField<?>> {
  ...
  private Change createChange(CtField<?> field, CtExpression<?> defaultExpression) {
    ...
    String markdown =
        "Field Initializer  `%s` for field `%s` in type `%s` was redundant and was removed.";
    MarkdownString changelog =
        MarkdownString.fromMarkdown(
            ...
            String.format(
                ...
                field.getDeclaringType().getQualifiedName()));
    return new Change(
        REDUNDANT_FIELD_INITIALIZATION, changelog, field.getDeclaringType().getTopLevelType());
  }
