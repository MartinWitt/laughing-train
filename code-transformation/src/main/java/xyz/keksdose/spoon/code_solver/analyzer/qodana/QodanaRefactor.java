...
/**
 * This aggregates all qodana refactorings and wraps them in a single processor. Use {@link
 * run(Path, ChangeListener)} to analyse the source code. <b> Note: </b> This requires a running
 * docker host and can take some minutes.
 */
public class QodanaRefactor extends TransformationProcessor<CtType<?>> {
  ...
  private QodanaRefactor(Builder builder) {
    ...
    this.ruleParser =
        builder.ruleParser.entrySet().stream()
            .collect(
                HashMap::new,
                (m, v) -> m.put(new RuleId(v.getKey()), v.getValue()),
                HashMap::putAll);
    this.settings.addAll(builder.settings);
  }

  public QodanaRefactor(
      Iterable<QodanaRules> qodanaRules,
      ChangeListener listener,
      List<? extends AnalyzerResult> results) {
    super(listener);
    refactorings = new ArrayList<>();
  ...
  private void splitResults(List<? extends AnalyzerResult> results) {
    for (AnalyzerResult result : results) {
      Optional.ofNullable(ruleParser.get(result.ruleID()))
          .ifPresent(v -> refactorings.add(v.apply(result)));
    }
  }
