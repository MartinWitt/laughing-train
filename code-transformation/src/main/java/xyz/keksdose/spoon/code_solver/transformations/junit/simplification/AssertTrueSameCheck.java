...
public class AssertTrueSameCheck extends TransformationProcessor<CtInvocation<?>> {
  ...
  public AssertTrueSameCheck(ChangeListener listener) {
    super(listener);
  }
  ...
  public void process(CtInvocation<?> invocation) {
    if (invocation.getExecutable() != null
        && JunitHelper.isJunit5AssertTrue(invocation.getExecutable())) {
      ...
      if (expression instanceof CtBinaryOperator) {
        ...
        if (binaryOperator.getKind().equals(BinaryOperatorKind.EQ)) {
          CtInvocation<?> junit5AssertNotNull =
  ...
  private void adjustImports(CtInvocation<?> element) {
    ...
    if (parent != null && !hasJunit5AssertTrueLeft(parent)) {
      ImportHelper.removeImport(
          "org.junit.jupiter.api.Assertions.assertTrue", true, compilationUnit);
    }
    ImportHelper.addImport("org.junit.jupiter.api.Assertions.assertSame", true, compilationUnit);
  }
  ...
  private CtInvocation<?> createJunit5AssertSame(
      CtExpression<?> firstArgument, CtExpression<?> secondArgument) {
    ...
    CtExecutableReference<?> assertEquals =
        getFactory()
            .Executable()
            .createReference(typeRef, voidType, "assertSame", List.of(objectType, objectType));
    return getFactory()
        .createInvocation(null, assertEquals, List.of(firstArgument, secondArgument));
  }
