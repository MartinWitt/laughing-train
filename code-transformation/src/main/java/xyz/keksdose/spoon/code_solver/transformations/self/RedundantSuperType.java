...
public class RedundantSuperType extends TransformationProcessor<CtType<?>> {
  ...
  public void process(CtType<?> element) {
    ...
    if (superClass == null
        || superClass.getTypeDeclaration() == null
        || element.getSuperInterfaces().isEmpty()) {
      return;
    }
    var oldImports = element.getPosition().getCompilationUnit().getImports();
    for (CtTypeReference<?> superInterface :
        Collections.unmodifiableSet(element.getSuperInterfaces())) {
