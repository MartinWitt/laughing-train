...
public class TestAnnotation extends TransformationProcessor<CtAnnotation<?>> {
  ...
  private static final String CHANGE_TEXT_MARKDOWN =
      "Replaced junit 4 test annotation with junit 5 test annotation in `%s`";
  private static final String CHANGE_TEXT_RAW =
      "Replaced junit 4 test annotation with junit 5 test annotation in %s";
  ...
  private static final CodeIssue TEST_RULE =
      new CodeIssue() {
        ...
        public MarkdownString getDescription() {
          String rawText =
              "The JUnit 4 @Test annotation should be replaced JUnit junit 5 Test annotation.";
          String markdownText =
    ...
    setChanged(
        ...
        new Change(
            ...
            MarkdownString.fromMarkdown(
                String.format(CHANGE_TEXT_RAW, getNameOfAnnotatedMethod(annotation)),
                String.format(
                    CHANGE_TEXT_MARKDOWN,
  ...
  private void refactorTimeoutAnnotation(CtAnnotation<?> annotation, CtElement element) {
    if (JunitHelper.hasTimeoutValue(annotation)) {
      ...
      timeout.addValue(
          "value", Long.valueOf(annotation.getValue("timeout").toString().replace("L", "")));
      ...
      ImportHelper.addImport(
          JAVA_UTIL_CONCURRENT_TIME_UNIT, false, element.getPosition().getCompilationUnit());
    }
  }
