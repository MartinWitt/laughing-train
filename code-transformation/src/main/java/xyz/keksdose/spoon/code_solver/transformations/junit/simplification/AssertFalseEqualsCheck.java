...
public class AssertFalseEqualsCheck extends TransformationProcessor<CtInvocation<?>> {
  ...
  public void process(CtInvocation<?> invocation) {
    if (invocation.getExecutable() != null
        && JunitHelper.isJunit5AssertFalse(invocation.getExecutable())) {
      ...
      if (expression instanceof CtInvocation) {
        ...
        if (equalsInvocation.getExecutable().getSimpleName().equals("equals")) {
          ...
          CtInvocation<?> junit5AssertEquals =
              createJunit5AssertNotEquals(firstArgument, secondArgument);
  ...
  private void adjustImports(CtInvocation<?> element) {
    ...
    if (parent != null && !hasJunit5AssertFalseLeft(parent)) {
      ImportHelper.removeImport(
          "org.junit.jupiter.api.Assertions.assertFalse", true, compilationUnit);
    }
    ImportHelper.addImport(
        "org.junit.jupiter.api.Assertions.assertNotEquals", true, compilationUnit);
  }
  ...
  private CtInvocation<?> createJunit5AssertNotEquals(
