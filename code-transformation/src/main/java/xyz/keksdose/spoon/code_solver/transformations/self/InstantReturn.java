      ...
      if (statement instanceof CtIf) {
        ...
        if (thenStatement instanceof CtBlock) {
          ...
          if (thenBlock.getStatements().size() == 1
              && thenBlock.getStatement(0) instanceof CtReturn) {
            CtReturn<?> ifReturnStatement = (CtReturn<?>) thenBlock.getStatement(0);
          }
        }
        // if(ifStatement.getThenStatement() instance.getStatements().size()==1 &&
        // ifStatement.getBody().getStatement(0) instanceof CTReturn) {

      }
    }
    // setChanged(element, new Change("Added return statement", "InstantReturn", element));
  }
  // setChanged(element.getDeclaringType(), new Change("Removed return statement", "InstantReturn",
