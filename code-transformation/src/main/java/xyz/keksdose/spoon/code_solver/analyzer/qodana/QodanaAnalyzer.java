...
public class QodanaAnalyzer {
  ...
  private List<AnalyzerResult> executeQodana(
      Path sourceRoot, DockerClient dockerClient, Image qodana) throws InterruptedException {
  ...
  private List<AnalyzerResult> startQodanaContainer(
      DockerClient dockerClient, CreateContainerResponse container) throws InterruptedException {
  ...
  private HostConfig createHostConfig(Path sourceRoot) {
    ...
    Bind resultsBind =
        new Bind(Path.of(resultFolder).toAbsolutePath().toString(), targetFile, AccessMode.rw);
