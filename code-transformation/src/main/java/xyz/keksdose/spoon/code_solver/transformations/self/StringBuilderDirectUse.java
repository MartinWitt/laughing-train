...
public class StringBuilderDirectUse extends TransformationProcessor<CtInvocation<?>> {
  ...
  private void notifyChangeListener(CtInvocation<?> element) {
    ...
    String markdownText =
        String.format("`%s` can be replaced by direct stringbuilder method", element);
    Change change =
        new Change(
            directStringBuilderUse, MarkdownString.fromMarkdown(rawText, markdownText), parent);
  ...
  private boolean targetIsStringBuilder(CtInvocation<?> element) {
    return Nullsafe.get(
        () -> element.getTarget().getType().getSimpleName().equals("StringBuilder"), false);
  }
