...
public class EmptyStringCheck extends TransformationProcessor<CtInvocation<?>> {
  ...
  public boolean isApplicable(CtInvocation<?> element) {
    if (element.getTarget() != null
        && isStringType(element.getTarget())
        && isEqualsMethod(element)) {
      if (isEmptyStringArgument(element)) {
        return true;
      } else {
        List<CtLiteral<?>> literals =
            element.getTarget().getElements(new TypeFilter<>(CtLiteral.class));
  ...
  private void notifyChangeListener(CtInvocation<?> invocation, CtExpression<?> target) {
    String rawText =
        String.format(
            "Empty String check was written as %s.equals(\"\") and refactored to %s.isEmpty()",
            target, target);
    String markdown =
        String.format(
            "Empty String check was written as `%s.equals(\"\")` and refactored to `%s.isEmpty()`",
