...
public class MethodMayBeStatic extends AbstractRefactoring {
  ...
  private static final BadSmell METHOD_MAY_BE_STATIC =
      new BadSmell() {
        @Override
  ...
  public void refactor(ChangeListener listener, CtType<?> type) {
    ...
    for (CtMethod<?> method : type.getMethods()) {
      if (getSourceStart(method) == getSourceStartOfResult()) {
        ...
        LinkedHashSet<CtExtendedModifier> modifiers =
            new LinkedHashSet<>(method.getExtendedModifiers());
      ...
      if (executableRef.getExecutable() != null) {
        ...
        if (exec instanceof CtMethod) {
          ...
          if (methodRef.equals(method)) {
            CtTypeReference<Object> typeReference =
                method
                    .getFactory()
