...
public class MissingOverride extends TransformationProcessor<CtMethod<?>> {
  ...
  public void process(CtMethod<?> element) {
    if (element.getTopDefinitions().size() == 1) {
      if (element.getAnnotations().stream().noneMatch(v -> v.getName().equals("Override"))) {
        if (element.getTopDefinitions().stream()
            .filter(v -> v.getBody() != null)
            .anyMatch(v -> !v.getBody().getStatements().isEmpty() || !v.isAbstract())) {
          CtAnnotation<?> overrideAnnotation =
              getFactory().createAnnotation(getFactory().Type().createReference(Override.class));
          ...
          setChanged(
              ...
              new Change(
                  String.format(
                      "Added missing override annotation to method %s at %s",
  ...
  /**
   * Modify an element such that the sniper printer detects it as modified, without changing its
   * final content. This forces it to be sniper-printed "as-is".
   */
