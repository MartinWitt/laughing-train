...
public class CollectionEmptyCheck extends TransformationProcessor<CtBinaryOperator<?>> {
  ...
  private void notifyChangeListener(
      CtBinaryOperator<?> element, CtInvocation<Boolean> innvocation) {
    ...
    setChanged(
        parent,
        new Change(COLLECTION_EMPTY_CHECK, MarkdownString.fromMarkdown(raw, markdown), parent));
  }
  ...
  private CtInvocation<Boolean> createIsEmptyInvocation(CtBinaryOperator<?> element) {
    ...
    return getFactory()
        .Code()
        .createInvocation(
            element.getElements(new TypeFilter<>(CtInvocation.class)).get(0).getTarget(),
            ref,
