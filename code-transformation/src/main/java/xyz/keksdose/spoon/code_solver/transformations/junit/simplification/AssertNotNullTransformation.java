...
public class AssertNotNullTransformation extends TransformationProcessor<CtInvocation<?>> {
  ...
  public void process(CtInvocation<?> element) {
    if (element.getExecutable() != null
        && JunitHelper.isJunit5AssertTrue(element.getExecutable())) {
  ...
  private CtInvocation<?> createJunit5AssertNotNull(CtExpression<?> check) {
    ...
    CtExecutableReference<?> assertNotNull =
        getFactory()
            .Executable()
            .createReference(
                typeRef, voidType, "assertNotNull", List.of(getFactory().Type().objectType()));
