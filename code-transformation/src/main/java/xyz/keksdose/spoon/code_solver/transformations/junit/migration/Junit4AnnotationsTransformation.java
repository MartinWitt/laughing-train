...
public class Junit4AnnotationsTransformation extends TransformationProcessor<CtMethod<?>> {
    ...
    private void refactorBeforeClass(CtMethod<?> method) {
        ...
        if (beforeClassAnnotation.isPresent()) {
            ...
            String changeText = String.format(
                    "Replaced @BeforeClass annotation with @BeforeAll at method %s", method.getSimpleName());
            String markdownText = String.format(
                    "Replaced `@BeforeClass` annotation with `@BeforeAll` at method `%s`", method.getSimpleName());
            notifyChangeListener(
                    method.getTopLevelType(), fromMarkdown(changeText, markdownText), JUNIT4_BEFORE_CLASS_BADSMELL);
        }
    }
    ...
    private void refactorBefore(CtMethod<?> method) {
        ...
        if (beforeAnnotation.isPresent()) {
            ...
            String changeText =
                    String.format("Replaced @Before annotation with @BeforeEach at method %s", method.getSimpleName());
            String markdownText = String.format(
    ...
    private void refactorAfter(CtMethod<?> method) {
        ...
        if (afterAnnotation.isPresent()) {
            ...
            String changeText =
                    String.format("Replaced @After annotation with @AfterEach at method %s", method.getSimpleName());
            String markdownText = String.format(
    ...
    private void refactorAfterClass(CtMethod<?> method) {
        ...
        if (afterClassAnnotation.isPresent()) {
            ...
            String markdownText = String.format(
                    "Replaced `@After` annotation with `@AfterEach` at method `%s`", method.getSimpleName());
            notifyChangeListener(
                    method.getTopLevelType(), fromMarkdown(changeText, markdownText), JUNIT4_AFTER_CLASS_BADSMELL);
        }
    }
    ...
    private void refactorIgnore(CtMethod<?> method) {
        ...
        if (ignoreAnnotation.isPresent()) {
            ...
            String changeText =
                    String.format("Replaced @Ignore annotation with @Disabled at method %s", method.getSimpleName());
            String markdownText = String.format(
