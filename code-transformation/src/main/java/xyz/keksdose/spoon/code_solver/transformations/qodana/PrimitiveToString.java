...
public class PrimitiveToString extends TransformationProcessor<CtBinaryOperator<?>> {
  ...
  private static final CodeIssue STRING_VALUE_OF =
      new CodeIssue() {
        ...
        public MarkdownString getDescription() {
          String raw =
              "Primitive types are converted to String using concationation with `\"\"`"
  ...
  private void refactor(CtBinaryOperator<?> op, CtExpression<?> primitive) {
    CtTypeReference<Object> stringType =
        getFactory().Type().createSimplyQualifiedReference("java.lang.String");
  ...
  private void notifyChangeListener(CtBinaryOperator<?> op, CtExpression<?> primitive) {
    ...
    setChanged(
        parent, new Change(STRING_VALUE_OF, MarkdownString.fromMarkdown(raw, markdown), parent));
  }
