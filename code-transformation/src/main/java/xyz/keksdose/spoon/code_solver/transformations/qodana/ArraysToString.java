...
public class ArraysToString extends TransformationProcessor<CtInvocation<?>> {
	...
	private static final BadSmell BAD_SMELL = new BadSmell() {
		...
		public MarkdownString getDescription() {
			String rawText = "array.toString() is not the best way to print an array. Use Arrays.toString(array) instead.";
			String markdownText = "`array.toString()` is not the best way to print an array. Use `Arrays.toString(array)` instead.";
	...
	private void replaceToStringCall(CtInvocation<?> invocation) {
		...
		CtInvocation<?> replacement = getFactory().Code()
	...
	private void adjustImports(CtInvocation<?> invocation) {
		ImportHelper.addImport("java.util.Arrays", false, invocation.getPosition().getCompilationUnit());
	}
	...
	private void notifyChangeListener(CtInvocation<?> invocation) {
		...
		setChanged(invocation.getParent(CtType.class),
