 ...
 * Can be configured to add or remove imports using {@link #setCanAddImports(boolean)} and {@link
 * #setCanRemoveImports(boolean)}. This does not force some references to be implicit, and doesn't
 * fix the wrong implicit which causes conflicts: this fixing done by {@link ImportConflictDetector}
 */
@Experimental
public class ImportCleaner extends ImportAnalyzer<ImportCleaner.Context> {
  ...
  protected void handleTargetedExpression(
      CtTargetedExpression<?, ?> targetedExpression, Context context) {
    if (context == null) {
      return;
    }
    ...
    if (target == null) {
      if (targetedExpression instanceof CtFieldAccess
          ...
          && ((CtFieldAccess<?>) targetedExpression)
              .getVariable()
              .getDeclaringType()
              .isSimplyQualified()) {
  ...
  protected void handleTypeReference(CtTypeReference<?> reference, Context context, CtRole role) {
    ...
    if (!reference.isImplicit() && reference.isSimplyQualified() || reference.isAnnotationType()) {
      /*
  ...
  public class Context {
    ...
    Context(CtCompilationUnit cu) {
      ...
      this.typeRefQNames =
          cu.getDeclaredTypeReferences().stream()
    ...
    void addImport(CtReference ref) {
      ...
      if (ref instanceof CtTypeReference
          && Objects.equals(packageQName, packageRef.getQualifiedName())
    ...
    private boolean isReferencePresentInImports(CtReference ref) {
      return compilationUnit.getImports().stream()
          .anyMatch(
              ctImport ->
                  ctImport.getReference() != null
                      && isEqualAfterSkippingRole(
                          ctImport.getReference(), ref, CtRole.TYPE_ARGUMENT));
    }
    ...
    private boolean isEqualAfterSkippingRole(CtElement element, CtElement other, CtRole role) {
      ...
      return isEqual || role == equalsVisitor.getNotEqualRole();
    }
    ...
    void onCompilationUnitProcessed(CtCompilationUnit compilationUnit) {
      ...
      for (CtImport oldImport : new ArrayList<>(existingImports)) {
        if (!removeImport(oldImport, computedImports)) {
          ...
          for (CtType type : compilationUnit.getDeclaredTypes()) {
            for (CtJavaDoc element : type.getElements(new TypeFilter<>(CtJavaDoc.class))) {
              for (CtJavaDocTag tag : element.getTags()) {
                // case @throws
                if (oldImport.getReference() != null
                    && oldImport.getReference().getSimpleName().equals(tag.getParam())) {
                  continue topfor;
                }
              }
              for (JavadocDescriptionElement part : (element).getJavadocElements()) {
                ...
                if (part instanceof JavadocInlineTag) {
                  ...
                  // @MartinWitt changed the approximation here
                  if (oldImport.getReference() != null
                      && content.contains(oldImport.getReference().getSimpleName())) {
                    continue topfor;
                  }
                }
              }
          ...
          if (oldImport.getImportKind() == CtImportKind.ALL_TYPES) {
            if (removeAllTypeImportWithPackage(
                computedImports,
                ((CtPackageReference) oldImport.getReference()).getQualifiedName())) {
              // this All types import still imports some type. Keep it
              continue;
            }
          }
          if (oldImport.getImportKind() == CtImportKind.ALL_STATIC_MEMBERS) {
            if (removeAllStaticTypeMembersImportWithType(
                computedImports,
                ((CtTypeMemberWildcardImportReference) oldImport.getReference())
                    .getTypeReference())) {
              // this All types import still imports some type. Keep it
              continue;
            }
      ...
      if (importComparator != null) {
        existingImports.set(
            existingImports.stream().sorted(importComparator).collect(Collectors.toList()));
      }
    }
  ...
  /**
   * @return fast unique identification of reference. It is not the same like printing of import,
   *     because it needs to handle access path.
   */
    ...
    for (Iterator<CtImport> iter = imports.iterator(); iter.hasNext(); ) {
      ...
      if (newImport.getImportKind() == CtImportKind.TYPE) {
        ...
        if (typeRef.getPackage() != null
            && packageName.equals(typeRef.getPackage().getQualifiedName())) {
          found = true;
          if (canRemoveImports) {
            iter.remove();
          }
   ...
   */
  private boolean removeAllStaticTypeMembersImportWithType(
      Set<CtImport> imports, CtTypeReference<?> typeRef) {
