...
public class ImportCleaner extends ImportAnalyzer<ImportCleaner.Context> {
	...
	public class Context {
		...
		void addImport(CtReference ref) {
			...
			if (ref instanceof CtTypeReference<?> && !isReferencePresentInImports(topLevelTypeRef)
					&& topLevelTypeRef != ref && !isReferencePresentInImports(ref)) {
			...
			if (ref instanceof CtTypeReference && Objects.equals(packageQName, packageRef.getQualifiedName())
			...
			topfor: for (CtImport oldImport : new ArrayList<>(existingImports)) {
				if (!removeImport(oldImport, computedImports)) {
					...
					if (oldImport.getImportKind() == CtImportKind.ALL_TYPES) {
						if (removeAllTypeImportWithPackage(computedImports,
					...
					if (oldImport.getImportKind() == CtImportKind.ALL_STATIC_MEMBERS) {
						if (removeAllStaticTypeMembersImportWithType(computedImports,
							((CtTypeMemberWildcardImportReference) oldImport.getReference()).getTypeReference())) {
							//this All types import still imports some type. Keep it
							continue;
						}
					}
	...
	private static boolean inheritsFrom(CtTypeReference<?> ref, CtTypeReference<?> potentialBase) {
		...
		return superClass != null && (superClass.getQualifiedName().equals(potentialBase.getQualifiedName())
				|| inheritsFrom(superClass, potentialBase));
	}

	private static boolean isStaticExecutableRef(CtElement element) {
		return element instanceof CtExecutableReference<?> && ((CtExecutableReference<?>) element).isStatic();
	}
}
