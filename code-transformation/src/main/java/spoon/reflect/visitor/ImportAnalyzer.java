...
/**
 * {@link Processor} of {@link CtCompilationUnit}, which scans CtCompilationUnit modules, packages
 * and types with purpose to find type references and expressions which might influence import
 * directives.
 *
 * <p>Subclasses create a scanner ({@link #createScanner()}) and analyzes the elements to be
 * imported {@link #handleTypeReference} and {@link #handleTargetedExpression(CtTargetedExpression,
 * Object)}
 */
@Experimental
public abstract class ImportAnalyzer<U> extends AbstractProcessor<CtElement> {
  ...
  protected static Set<CtRole> IGNORED_ROLES_WHEN_IMPLICIT =
      new HashSet<>(
          Arrays.asList(
  ...
  /**
   * {@link CtScannerListener} implementation which stops scanning of children on elements, which
   * mustn't have influence to compilation unit imports.
   */
  ...
  protected void onEnter(U context, CtRole role, CtElement element) {
    ...
    if (element instanceof CtTargetedExpression) {
      ...
      // we have to visit only PURE CtTypeReference. No CtArrayTypeReference,
      // CtTypeParameterReference, ...
      element.accept(
          new CtAbstractVisitor() {
            @Override
  ...
  protected abstract void handleTargetedExpression(
      CtTargetedExpression<?, ?> targetedExpression, U context);
