...
public interface BadSmell {
  ...
  /**
   * Fixes the bad smell. Fixing means changing the source code in a way that the bad smell is not
   * present anymore. This method is not supported by default. Call {@link #isFixable()} to check if
   * the bad smell is fixable.
   */
  default void fix() {
