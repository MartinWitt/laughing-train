...
public class SizeReplaceableByIsEmptyAnalyzer
    implements LocalAnalyzer, LocalRefactor<SizeReplaceableByIsEmpty> {
  ...
  public List<BadSmell> analyze(CtType<?> clazz) {
    ...
    for (CtBinaryOperator<?> ctBinaryOperator : elements) {
      ...
      if (sizeInvocation != null && zeroLiteral != null) {
        badSmells.add(
            new SizeReplaceableByIsEmpty(clazz, ctBinaryOperator, sizeInvocation, zeroLiteral));
      }
    }
    return badSmells;
  }
  ...
  public void refactor(SizeReplaceableByIsEmpty badSmell) {
    ...
    CtExecutableReference<?> isEmpty =
        factory.Executable()
            .createReference(target.getType(), false, factory.Type().BOOLEAN, "isEmpty");
