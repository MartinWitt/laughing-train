...
public class App {
  ...
  @Inject UserWhitelist userWhitelist;

  @Inject QodanaService qodanaService;
  ...
  void onConfigEdit(@Issue.Edited GHEventPayload.Issue issueComment) throws IOException {
    ...
    if (isNotConfigIssue(issueComment)
      ...
      logger.atInfo().log(
          "Ignoring config edit because it is not a config issue or it is from self");
      return;
    }
    ...
    if (containsFlag(issueComment.getIssue(), DISABLE_ALL_RULES_BUTTON)) {
      issueComment
          .getIssue()
          .setBody(refreshFlag(issueComment.getIssue().getBody(), DISABLE_ALL_RULES_BUTTON));
      disableAllRules();
      issueComment.getIssue().setBody(config.regenerateConfig());
    }
  }
  ...
  private void refreshConfig(GHEventPayload.Issue issueComment) throws JsonProcessingException {
    ...
    @Var
    String newConfig =
        body.substring(body.indexOf("<!-- config-start -->"), body.indexOf("<!-- config-end -->"));
  ...
  private void createPullRequestForAffectedType(
    ...
    logger.atInfo().log("Found changes for %s types", changesByType.entrySet().size());
    if (changesByType.entrySet().size() > config.getMaximumNumberOfPrs()) {
      logger.atInfo().log("Too many changes, skipping, %s", changesByType.entrySet().size());
      return;
    }
  ...
  private void createSinglePullRequest(GHRepository repo, Path dir, List<? extends Change> changes)
    ...
    createCommit(
        repo, dir, changes.stream().map(Change::getAffectedType).collect(Collectors.toList()), ref);
  ...
  private void createPullRequest(GHRepository repo, String typeName, String branchName, String body)
    ...
    repo.createPullRequest(
            "fix Bad Smells in " + typeName, branchName, repo.getDefaultBranch(), body)
        .addLabels(LABEL_NAME);
  }

  private void createPullRequest(GHRepository repo, String branchName, String body)
      throws IOException {
    repo.createPullRequest("fix Bad Smells", branchName, repo.getDefaultBranch(), body)
        .addLabels(LABEL_NAME);
  }

  private void createCommit(GHRepository repo, Path dir, CtType<?> entry, GHRef ref)
      throws IOException {
    var tree =
        repo.createTree()
