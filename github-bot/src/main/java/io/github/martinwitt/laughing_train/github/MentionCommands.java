...
public class MentionCommands {
  ...
  void mentionCommands(@IssueComment GHEventPayload.IssueComment issueComment) throws IOException {
    ...
    if (comment.contains("@laughing-train close")) {
      closePullRequestsWithLabelName(
          GitHubUtils.getOpenPullRequests(issueComment), Constants.LABEL_NAME);
      return;
    }
    if (comment.contains("@laughing-train hi")) {
      issueComment
          .getIssue()
          .comment("Hi, I'm a bot. I'm here to help you with your code quality.");
    }
  }

  private List<GHIssue> findConfigIssue(GHEventPayload.IssueComment issueComment)
      throws IOException {
    return issueComment
        .getRepository()
        .queryIssues()
        .creator(Constants.BOT_NAME)
        .list()
        .toList()
        .stream()
  ...
  private void createConfigIssue(GHRepository repository) throws IOException {
    repository.createIssue(Constants.CONFIG_ISSUE_NAME).body(config.regenerateConfig()).create();
  }
}
